...
from AudioTranscription import DetectHotword, init, isRunningOnCpu
...
def PrintHeader():
    ...
    if (isRunningOnCpu()):
        print("WARNING: Hotword detection is running on CPU. Please make sure CUDA is installed.")
...
def Initialize():
    ...
    modelLoadSuccess = init()
    if (modelLoadSuccess):
        print("> Sucessfully loaded Speech-To-Text model.")
    else:
        print("Failed to load Speech-To-Text model.")
        exit()
